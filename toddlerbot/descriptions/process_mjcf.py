...
def add_hanging(root: ET.Element, general_config: Dict[str, Any]):
    # add four hanging sites on the torso
    ...
    hanging_names = ["left_front", "left_back", "right_front", "right_back"]
    if torso is None:
        raise ValueError("No torso element found in the XML.")
    for i, offset in enumerate(hanging_offset):
        ET.SubElement(
            torso,
            "site",
            {
                "name": f"torso_{hanging_names[i]}_site",
                "pos": f"{offset[0]} {offset[1]} 0.122",
                "size": "0.005",
            },
        )
    # add four anchor bodies, each with two sliding joints in x and y direction
    for i, offset in enumerate(hanging_offset):
        anchor = ET.Element(
            "body",
        ...
        ET.SubElement(
            anchor,
            "joint",
            name=f"hang_{hanging_names[i]}_x",
            type="slide",
            axis="1 0 0",
        )
        ET.SubElement(
            anchor,
            "joint",
            name=f"hang_{hanging_names[i]}_y",
            type="slide",
            axis="0 1 0",
        )
        ET.SubElement(
            anchor,
            "site",
            name=f"anchor_{hanging_names[i]}_site",
            pos="0 0 0",
            size="0.003",
            rgba="0 0 1 1",
        )
        root.find(".//worldbody").append(anchor)  # type: ignore

    # add spacial tendon between each anchor and torso site with class "anchor_tendon"
    tendon = root.find(".//tendon")
    assert tendon is not None
    for i, offset in enumerate(hanging_offset):
        hang_tendon = ET.SubElement(
            tendon,
            "spatial",
            {"name": f"tendon_{hanging_names[i]}", "class": "anchor_tendon"},
        )
        ET.SubElement(hang_tendon, "site", site=f"anchor_{hanging_names[i]}_site")
        ET.SubElement(hang_tendon, "site", site=f"torso_{hanging_names[i]}_site")

    # add hanging motor on each tendon with class "hang_motor"
    # add position motors for each anchor slide joint with class "slide_anchor_motor"
    actuator = root.find(".//actuator")
    ...
    for i, offset in enumerate(hanging_offset):
        ET.SubElement(
            ...
            "motor",
            {
                "name": f"hang_{hanging_names[i]}",
                "tendon": f"tendon_{hanging_names[i]}",
                "class": "hang_motor",
            },
        )
        for axis in ["x", "y"]:
            ET.SubElement(
                ...
                "position",
                {
                    "name": f"hang_{hanging_names[i]}_{axis}",
                    "joint": f"hang_{hanging_names[i]}_{axis}",
                    "class": "slide_anchor_motor",
                },
            )

    # append anchor body home position to the qpos of keyframe's "home" key
    keyframe = root.find(".//keyframe")
    assert keyframe is not None
    key = keyframe.find(".//key[@name='home']")
    assert key is not None
    key.set(
        "qpos",
        key.get("qpos") + " 0.022 0.045 -0.022 0.045 0.022 -0.045 -0.022 -0.045",  # type: ignore
    )

    # define the default classes
    default = root.find(".//default")
    assert default is not None
    default_cls = ET.SubElement(default, "default", {"class": "slide_anchor_motor"})
    ET.SubElement(default_cls, "position", kp="0.1", dampratio="1")
    default_cls = ET.SubElement(default, "default", {"class": "anchor_tendon"})
    ET.SubElement(
        default_cls,
        "tendon",
        limited="true",
    ...
    default_cls = ET.SubElement(default, "default", {"class": "hang_motor"})
    ET.SubElement(default_cls, "motor", ctrlrange="-8 0")
...
def get_mjcf_files(robot_name: str, generate_hanging: bool = False):
    ...
    if not robot.config["general"]["is_fixed"]:
        ...
        if generate_hanging:
            add_hanging(xml_root, robot.config["general"])
            mjcf_hanging_path = os.path.join(robot_dir, robot_name + "_hang.xml")
            xml_tree.write(mjcf_hanging_path)
            create_scene_xml(mjcf_hanging_path, is_fixed=False)
            print(f"created xml at {mjcf_hanging_path}")
