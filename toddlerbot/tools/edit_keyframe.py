...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=str,
        default="ladder",
        help="The name of the scene. Need to match the name in descriptions.",
    )
    parser.add_argument(
        "--task",
        type=str,
    ...
    if len(args.scene) > 0:
        scene_xml_path = os.path.join(
            ...
            "descriptions",
            args.robot,
            f"{args.robot}_{args.scene}_scene.xml",
        )
        if not os.path.exists(scene_xml_path):
            print(f"Scene XML file not found: {scene_xml_path}")
            scene_xml_path = ""

    app = QApplication()

    robot = Robot(args.robot)
    sim = MuJoCoSim(robot, vis_type="none", xml_path=scene_xml_path)
