...
class MuJoCoApp:
    def __init__(self, root, robot: Robot):
        ...
        self.keyframe_dir = "toddlerbot/key_frames"
        self.qpos_offset = 6
        self.create_widgets()
        self.load_keyframes()
        self.joint_sliders: Dict[int, Any] = {}
        self.load_xml()
    ...
    def create_widgets(self):
        ...
        self.record_button = tk.Button(
            top_frame, text="Record Keyframe", command=self.record_keyframe
        )
        ...
        self.store_trajectory_button = tk.Button(
            top_frame, text="Store Trajectory", command=self.store_trajectory
        )
        ...
        self.load_button = tk.Button(
            self.button_frame, text="Load Keyframe", command=self.load_keyframe
        )
        self.load_button.grid(row=0, column=0, padx=5, pady=5)

        self.rename_button = tk.Button(
            self.button_frame, text="Rename Keyframe", command=self.rename_keyframe
        )
        self.rename_button.grid(row=0, column=1, padx=5, pady=5)

        self.delete_button = tk.Button(
            self.button_frame, text="Delete Keyframe", command=self.delete_keyframe
        )
        self.delete_button.grid(row=0, column=2, padx=5, pady=5)

        self.update_button = tk.Button(
            self.button_frame, text="Update Keyframe", command=self.update_keyframe
        )
        ...
        self.pause_button = tk.Button(
            self.button_frame, text="Start Simulation", command=self.toggle_simulation
        )
        self.pause_button.grid(row=0, column=4, padx=5, pady=5)

        self.mirror_checked = tk.BooleanVar()
        self.mirror_checkbox = tk.Checkbutton(
            self.button_frame, text="Mirror", variable=self.mirror_checked
        )
        ...
        self.rev_mirror_checkbox = tk.Checkbutton(
            self.button_frame, text="Rev. Mirror", variable=self.rev_mirror_checked
        )
        ...
        self.add_to_sequence_button = tk.Button(
            self.button_frame, text="Add to Sequence", command=self.add_to_sequence
        )
        ...
        self.interpolation_steps_label = tk.Label(
            self.button_frame, text="Interpolation Steps:"
        )
        ...
        self.exit_button = tk.Button(
            self.button_frame, text="Exit", command=self.root.quit
        )
    ...
    def update_sliders(self):
        for i in range(self.model.njnt):
            joint_slider = self.joint_sliders_frame.grid_slaves(
                row=i // self.slider_columns,
                column=(i % self.slider_columns) * (self.slider_columns - 1) + 1,
            )
            if joint_slider:
    ...
    def create_joint_sliders(self):
        ...
        for i in range(self.model.njnt):
            ...
            if (
                joint_name is None
                or "act" in joint_name
                or "rev" in joint_name
                or ("drive" in joint_name and "driven" not in joint_name)
            ):
            ...
            slider = tk.Scale(
                ...
                resolution=0.01,
                length=150,
                command=lambda val, idx=i: self.update_joint_position(
                    idx, val, self.qpos_offset
                ),
            )
            slider.grid(
                row=row, column=column * (self.slider_columns - 1) + 1, sticky="ew"
            )
            ...
            slider.bind(
                "<Double-Button-1>",
                lambda event, idx=i + self.qpos_offset: self.reset_joint(idx),
            )
    ...
    def create_joint_slider(self, qpos_index, joint_range, joint_name):
        ...
        # Double-click event binding to reset joint to initial position
        slider.bind(
            "<Double-Button-1>", lambda event, idx=qpos_index: self.reset_joint(idx)
        )
    ...
    def update_joint_position(self, joint_index, value, qpos_offset=0):
        ...
        if mirror_checked or rev_mirror_checked:
            joint_name = mujoco.mj_id2name(
                self.model, mujoco.mjtObj.mjOBJ_JOINT, joint_index
            )
            if "left" in joint_name or "right" in joint_name:
                ...
                mirrored_joint_index = mujoco.mj_name2id(
                    self.model, mujoco.mjtObj.mjOBJ_JOINT, mirrored_joint_name
                )
                self.data.qpos[mirrored_joint_index + self.qpos_offset] = (
                    mirror_checked * float(value) - rev_mirror_checked * float(value)
                )
        mujoco.mj_forward(self.model, self.data)
        self.viewer.sync()
    ...
    def load_keyframes(self):
        ...
        for filename in keyframe_files:
            if filename.endswith(".txt"):
                ...
                index = int(index_str)
                keyframe_path = os.path.join(self.keyframe_dir, filename)
                with open(keyframe_path, "r") as f:
                    qpos = [float(line.strip()) for line in f.readlines()]
                    self.keyframes.append(
                        Keyframe(index, keyframe_name, np.array(qpos))
                    )
    ...
    def record_keyframe(self):
        ...
        keyframe_name = (
            self.keyframe_name_entry.get() or f"Keyframe {self.current_index}"
        )
        keyframe = Keyframe(
            self.current_index,
            keyframe_name,
            self.data.qpos.copy()[self.qpos_offset :],
        )
        ...
        keyframe_path = os.path.join(
            self.keyframe_dir, f"{self.current_index}_{keyframe_name}.txt"
        )
        ...
        if hasattr(self, "selected_keyframe"):
            ...
            if new_name:
                old_file_path = os.path.join(
                    self.keyframe_dir, f"{index}_{old_name}.txt"
                )
                new_file_path = os.path.join(
                    self.keyframe_dir, f"{index}_{new_name}.txt"
                )
    ...
    def delete_keyframe(self):
        ...
        if hasattr(self, "selected_keyframe") and self.keyframe_listbox.curselection():
            ...
            os.remove(os.path.join(self.keyframe_dir, keyframe_file))
            ...
            for i in range(self.selected_keyframe, len(self.keyframes)):
                ...
                old_file_path = os.path.join(
                    self.keyframe_dir, f"{old_index}_{old_name}.txt"
                )
                new_file_path = os.path.join(self.keyframe_dir, new_name)
    ...
    def update_keyframe(self):
        if hasattr(self, "selected_keyframe"):
            index, keyframe_name, _ = self.keyframes[self.selected_keyframe]
            self.keyframes[self.selected_keyframe] = (
                index,
                keyframe_name,
                self.data.qpos.copy()[self.qpos_offset :],
            )
            ...
            keyframe_path = os.path.join(
                self.keyframe_dir, f"{index}_{keyframe_name}.txt"
            )
            with open(keyframe_path, "w") as f:
    ...
    def add_to_sequence(self):
        ...
        if selected_index:
            ...
            self.sequence_list.append(
                (keyframe_name, float(self.arrival_time_entry.get()))
            )
            self.update_sequence_listbox()

    def update_arrival_time(self):
        if hasattr(self, "selected_sequence"):
            keyframe_name = self.sequence_list[self.selected_sequence][0].rsplit(
                " ", 1
            )[0]
            arrival_time = float(self.arrival_time_entry.get())
            self.sequence_list[self.selected_sequence] = (
                f"{keyframe_name} {arrival_time}",
                arrival_time,
            )
            self.update_sequence_listbox()
    ...
    def generate_trajectory(self):
        ...
        for keyframe_name, arrival_time in self.sequence_list:
            ...
            index = next(
                i
                for i, (i, name, _) in enumerate(self.keyframes)
        ...
        self.trajectory_times = np.array(
            [
                ...
                for t in np.linspace(
                    0,
                    float(self.total_time_entry.get()),
                    int(self.interpolation_steps_entry.get()),
                )
            ]
        )
    ...
    def store_trajectory(self):
        ...
        np.savez(
            f"toddlerbot/ref_motion/{trajectory_name}",
            time=self.trajectory_times,
            action=motor_action,
        )
    ...
    def display_trajectory(self):
        ...
        for qpos in self.trajectory:
            ...
            self.viewer.sync()
            self.root.after(
                int(float(self.total_time_entry.get()) * 1000 / len(self.trajectory))
            )
