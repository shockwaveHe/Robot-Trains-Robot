...
class OnPolicyRunner:
    ...
    def __init__(
        self, env: VecEnv, train_cfg: dict, run_name: str | None = None, device="cpu"
    ):
        self.cfg = train_cfg
        ...
        if "autoencoder_cfg" in self.alg_cfg:
            latent_dim = self.alg_cfg["autoencoder_cfg"]["model"]["n_embd"] * 2
            num_obs += latent_dim
    ...
    def learn(self, num_learning_iterations: int, init_at_random_ep_len: bool = False):  # noqa: C901
        ...
        if self.alg.autoencoder is not None:
            ...
            # recon_loss = 0.0
            for batch in self.alg.params_dataloader:
                z, mean, logvar = self.alg.autoencoder.encode(batch)
            ...
            if self.log_dir is not None:
                ...
                if it % self.save_interval == 0:
                    self.save(
                        os.path.join(
                            self.log_dir,
                            f"model_{it}_r={round(self.latest_mean_reward)}.pt",
                        )
                    )
        ...
        if self.log_dir is not None:
            self.save(
                os.path.join(
                    self.log_dir,
                    f"model_{self.current_learning_iteration}_r={round(self.latest_mean_reward)}.pt",
                )
            )
    ...
    def log(self, locs: dict, width: int = 80, pad: int = 35):
        ...
        if len(locs["rewbuffer"]) > 0:
            ...
            self.latest_mean_reward = statistics.mean(locs["rewbuffer"])
    ...
    def save(self, path: str, infos=None):
        ...
        heapq.heappush(self.best_ckpts, (self.latest_mean_reward, path))
