...
from toddlerbot.locomotion.actor_critic import ActorCritic, ActorCriticRMA
...
class PPO:
    ...
    def __init__(
        ...
        learning_stage: str = "1",
        action_frame_stack: int = 1
    ):
        self.device = device
        ...
        self.action_frame_stack = action_frame_stack
        # RND components
    ...
    def build_autoencoder(self, autoencoder_cfg: dict):
        ...
        if model_cfg["dynamics_type"] == "hyper":
            params_dataset = HyperparameterDataset(data_cfg)
        else:
            raise ValueError(
                f"Invalid dynamics type: {model_cfg['dynamics_type']}"
            )
        # normalize the parameters
        if data_cfg["normalize_params"]:
            params_dataset.normalize_params()
        
        if autoencoder_cfg["model"]["dynamics_type"] == "hyper":
            input_splits = [params_dataset.params.shape[1]]
        elif autoencoder_cfg["model"]["dynamics_type"] == "params":
            ...
            raise ValueError(
                f"Invalid dynamics type: {autoencoder_cfg['model']['dynamics_type']}"
            )
    ...
    def act(self, obs, critic_obs, actions):
        if self.actor_critic.is_recurrent:
            self.transition.hidden_states = self.actor_critic.get_hidden_states()
        # Compute the actions and values
        self.transition.actions, action_real = self.actor_critic.act(obs, actions)
    ...
    def process_env_step(self, rewards, dones, infos):
        ...
        if "action_history" in infos:
            self.transition.actions = infos["action_history"].clone()
    ...
    def update(self):  # noqa: C901
        ...
        for (
            ...
            if self.autoencoder is not None:
                if self.learning_stage == "1":
                    ...
                    target_latents = self.get_infer_latents()
                    target_latents = target_latents[env_indices_batch]
                    actions_batch = actions_batch.reshape(-1, self.action_frame_stack, actions_batch.shape[-1] // self.action_frame_stack)
                    all_latents = self.actor_critic.get_latent(obs_batch[:, :-self.latent_dim], actions_batch)
                    ...
                    continue
                    # slice action_batch for only the first one
            ...
            self.actor_critic.act(
                obs_batch, actions_batch, masks=masks_batch, hidden_states=hid_states_batch[0]
            )
        ...
        if mean_adaptation_loss is not None:
            mean_adaptation_loss = np.mean(mean_adaptation_loss)
        # -- Clear the storage
        self.storage.clear()
        if self.learning_stage == "1":
            return (
