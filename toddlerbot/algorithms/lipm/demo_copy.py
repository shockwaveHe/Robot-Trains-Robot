...
import numpy as np
from lipm_3d_copy import LIPM3DPlanner
...
LIPM_model = LIPM3DPlanner(dt=delta_t, T_sup=0.5)
...
for i in range(int(total_time / delta_t)):
    ...
    if (i > 0) and (i % switch_index == 0):
        ...
        LIPM_model.switch_support_leg()  # switch the support leg
        step_num += 1
        ...
        x_0, vx_0, y_0, vy_0 = LIPM_model.calculate_Xt_Vt(
            LIPM_model.T_sup
        ...
        if LIPM_model.support_leg == "left_leg":
            ...
            y_0 = (
                y_0 + LIPM_model.left_foot_pos[1]
            )  # need the absolute position for next step
        else:
            x_0 = (
                x_0 + LIPM_model.right_foot_pos[0]
            )  # need the absolute position for next step
            y_0 = (
                y_0 + LIPM_model.right_foot_pos[1]
            )  # need the absolute position for next step

        LIPM_model.calculate_foot_location_for_next_step(
...
ani_3D = FuncAnimation(
    fig,
    ani_3D_update,
    frames=range(1, data_len),
    init_func=ani_3D_init,
