...
python toddlerbot/tools/sysID/collect_data.py --robot-name sysID_XC430 --joint-names joint_0 --n-trials $N_TRIALS --n-loads $N_LOADS --exp-folder-path "$EXP_FOLDER_PATH"
...
for ((i = N_LOADS-1; i >= 0; i--)); do
    ...
    read -r is_ready
    if [ "$is_ready" == "y" ]; then
        python toddlerbot/tools/sysID/collect_data.py --robot-name sysID_XC430 --joint-names joint_0 --n-trials $N_TRIALS --n-loads "$i" --exp-folder-path "$EXP_FOLDER_PATH"
    else
